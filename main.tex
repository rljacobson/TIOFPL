...
% For the inference rules in chapter 8
...
\usepackage{caption}
\captionsetup{labelsep=quad,font={sf,footnotesize}}
...
\newcommand{\combinatorbox}[3][0.5\textwidth]{%
    \par
    {\centering
        \vspace{0.5\baselineskip}
        \setlength{\fboxrule}{1pt}%
...
% The following is useful for authoring but needs to be commented for final compilation. It excludes all other files but still reads in their aux files so that counters, etc., remain valid.
\includeonly{chapters/chap14}
...
\part[Graph Reduction]{Graph Reduction}
...
% Adjust the `\includeonly{..}` command before `\begin{document}` to include/exclude document content for faster compilation.
